...
def get_output_path_from_config(config_path: str, config: dict) -> str:
    ...
    out_fname = os.path.split(config_path)[-1] + "_" + str(uuid.uuid4()) + ".json"
...
def get_polygon_context(path, extent_area=True):
    ...
    if not extent_area:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            area_ids = [
                PolygonContext(float(i.GetField("area")), i.GetField("UID"), i.GetField("state"))
                for i in layer
            ]
    else:
...
def filter_polygons_by_context(
    contexts: [PolygonContext], output_path: str, missing_only: bool, filter_state: str or None
):
    """Filter polygons based on their properties."""
    state_filtered = [c for c in contexts if not filter_state or c.state == filter_state]
...
def alloc_chunks(contexts, n_chunks):
    ...
    out = [
        {"max_mem_Mi": max(est_mem(i) for i in chunk) if chunk else 0, "ids": chunk}
...
def main(config_path, n_chunks):
    ...
    filtered = filter_polygons_by_context(contexts, config["OUTPUTDIR"], missing_only, filter_state)
